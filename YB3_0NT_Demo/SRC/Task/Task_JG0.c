...
uint8 	g_au8JG0_3Buff[10][831];  //激光0数据缓存3个；
int32   LMS_data_0[10][365]={0};
uint8   JG0_CurBuff[831]={0};
uint8   g_au8recvBuff[1470];
...
//				SETUPALIAS.u32Net1_InvalidRecNum++;
			...
			{	  
				...
				g_u8JG0_RBuff_Count=g_u8JG0_RBuff_Count%10;
				g_u32count0++;
			}	
#else 
		    if(i!=1460)
			...
			{
				memcpy(g_au8recvBuff , Rx_Buffer,1460);
//				g_u8JG0_PBuff_Count  = 1;
//				g_u8JG1_PBuff_Count  = 1;
#ifdef 	   SIM_SOFTWARE
				g_u8JG0_RBuff_Count = 0;
				g_u8JG1_RBuff_Count = 0;
#endif
				for(i=85,j=0;i<806&&j<POINT_SUM;i=i+2,j++)
				{		
					LMS_data_0[g_u8JG0_RBuff_Count][j]=	(g_au8recvBuff[i-71]<<8)+g_au8recvBuff[i-70];
					LMS_data_1[g_u8JG1_RBuff_Count][j]=	(g_au8recvBuff[i+651]<<8)+g_au8recvBuff[i+652];	
				}  
				LMS_data_0[g_u8JG0_RBuff_Count][361] = ((g_au8recvBuff[8]<<24)+(g_au8recvBuff[9]<<16)+(g_au8recvBuff[10]<<8)+g_au8recvBuff[11]);				
				LMS_data_1[g_u8JG1_RBuff_Count][361] = ((g_au8recvBuff[8]<<24)+(g_au8recvBuff[9]<<16)+(g_au8recvBuff[10]<<8)+g_au8recvBuff[11]);
				l_u32time =  t0_count2;
				LMS_data_1[g_u8JG1_RBuff_Count][362]=l_u32time;
				LMS_data_0[g_u8JG0_RBuff_Count][362]=l_u32time;
				
#ifndef SIM_SOFTWARE			
				g_u8JG0_RBuff_Count=g_u8JG0_RBuff_Count+1;
				g_u8JG0_RBuff_Count=g_u8JG0_RBuff_Count%10;
				g_u8JG1_RBuff_Count=g_u8JG0_RBuff_Count;
#else
			 	g_u8JG0_RBuff_Count = 1;
				g_u8JG1_RBuff_Count = 1;
#endif
